...
def fetch():
    ...
    pdiff=diff
    weight_delt=prog*(weight_delt)+5*np.random.rand(10,3)*3*sigmoid(-0.05*steps)
    weights+=weight_delt
    ans=list(sigmoid(diff.T.dot(weights)))
    print(ans)
    ...
    if token:
        sp = spotipy.Spotify(auth=token)
        sp.trace = False
        sp.next_track(device_id='cd7b87d843045401893afa4f39d91f9bbb7161b0')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ip", default="10.19.187.189",help="The ip to listen on")
